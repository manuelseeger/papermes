...
from lib.firefly_client import FireflyClient, FireflyAPIError
...
class TestFireflyClient:
    ...
    def test_client_initialization_missing_token(self):
        ...
        with patch.dict(os.environ, {"PAPERMES_FIREFLY_HOST": "http://test.com"}, clear=True):
    ...
    def test_connection_and_accounts_retrieval(self, firefly_client):
        ...
        for account in accounts.data[:5]:  # Show first 5 accounts
            ...
            print(f"   - {account.attributes.name} ({account.attributes.type}): {balance} {currency}")
    ...
    def test_asset_accounts_filtering(self, firefly_client):
        ...
        for account in asset_accounts.data:
            assert account.attributes.type in ["asset", "Default account", "Cash account", "Savings account"], \
    ...
    def test_pagination(self, firefly_client):
        ...
        if len(all_accounts.data) > 2:
            ...
            assert page_1_ids.isdisjoint(page_2_ids), "Page 1 and 2 should have different accounts"
    
    @pytest.mark.parametrize("account_type", ["asset", "expense", "revenue", "liability"])
...
class TestFireflyClientTransactionCreation:
    ...
    def test_create_withdrawal_transaction(self, firefly_client, sample_account_id):
        """
    ...
    @pytest.mark.creates_data
    def test_create_transaction_with_group(self, firefly_client, default_account_id, sample_account_id):
        """
